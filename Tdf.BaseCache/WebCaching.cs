...
{
    /// <summary>
    /// Url:http://www.cnblogs.com/hanyinglong/p/Cache.html
    /// HttpRuntime Cache读取设置缓存信息封装
    /// 使用描述：给缓存赋值使用WebCaching.Set(key,value....)等参数
    /// (第三个参数可以传递文件的路径(HttpContext.Current.Server.MapPath()))
    /// 读取缓存中的值使用JObject jObject=WebCaching.Get(key) as JObject，
    /// 读取到值之后就可以进行一系列判断
    /// </summary>
    public class WebCaching : ICacheManager
    {
        /// <summary>
        /// 设置缓存时间，配置(从配置文件中读取)
        /// </summary>
        ...
        {
            Set(key, value, null, DateTime.Now.AddMinutes(cacheTime), Cache.NoSlidingExpiration,
        ...
        /// 缓存指定对象，设置缓存
        /// </summary>
        public bool Set(string key, object value, CacheDependency cacheDependency)
        {
        ...
        /// 缓存指定对象，设置缓存
        /// </summary>
        public bool Set(string key, object value, double seconds, bool isAbsulute)
        {
